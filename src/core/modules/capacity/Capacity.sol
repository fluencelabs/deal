  ...
  ICapacity
{
  ...
  bytes32 constant gNonceHardcoded =
  ...
  function checkProofs(
    ...
    require(unitIds.length > 0, "No proofs given");
    require(
      unitIds.length == localUnitNonces.length,
      "Invalid local nonces number"
    );
    require(
      unitIds.length == resultHashes.length,
      "Invalid result hashes number"
    );
    ...
    for (uint256 i = 0; i < proofsLength; i++) {
      globalUnitNonces[i] = keccak256(
        abi.encodePacked(gNonceHardcoded, unitIds[i])
      );
    }
  ...
  function _preCommitCommitmentSnapshot(
    ...
    if (
      storageStatus != CCStatus.Active || snapshotEpoch <= lastSnapshotEpoch
    ) {
