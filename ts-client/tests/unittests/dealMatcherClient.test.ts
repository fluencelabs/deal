...
import { IndexerClient } from "../../src/dealMatcherClient/indexerClient/indexerClient";
import { ethers } from "ethers";
import { OffersQueryQuery } from "../../src/dealMatcherClient/indexerClient/queries/offers-query.generated";

async function createRandomIndexerOffersData(
  offers: number,
  peers: number,
  computeUnits: number,
...
describe("#getMatchedOffers", () => {
  async function _callImpl(callImplProps: CallImplProps) {
    ...
    const data = createRandomIndexerOffersData(
      callImplProps.offersMockedPage1,
      callImplProps.peersMockedPage1,
      callImplProps.CUsMockedPage1,
    );
    // TODO: add possibility to add compute units to the same offer.
    const data2 = createRandomIndexerOffersData(
      callImplProps.offersMockedPage2,
      callImplProps.peersMockedPage2,
      callImplProps.CUsMockedPage2,
    );
    IndexerClient.prototype.getOffers = vi
      .fn()
    ...
    expect(_getMatchedOffersPageSpy.mock.calls.length).toEqual(
      callImplProps.exceptedIndexerCalls,
    );
