...
export function updateProviderFixtureAddress(address: string, providers: Array<ProviderFixtureModel>) {
...
export async function createCommitmentForProviderFixtures(
  ...
  const allPeerIds: string[] = providers.flatMap((provider) => provider.peerIds);
...
export async function createDealsFromFixtures(
  ...
  minDealDepositedEpoches: bigint,
  wait_confirmations: number,
) {
  for (const dealFixture of deals) {
    ...
    const approveTx = await paymentToken.approve(dealFactoryAddress, toApproveFromDeployer);
