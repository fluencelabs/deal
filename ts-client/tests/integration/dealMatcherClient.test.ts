...
import {
  ContractsENV,
  DealClient,
  DealMatcherClient, ICapacity, ICore,
  IDealFactory, IERC20, IMarket
} from "../../src";
import {
  ethers,
  HDNodeWallet,
...
import {
  ...
  createDealsFromFixtures, depositCollateral,
  getMarketExampleFixture,
  registerMarketOffersFromFixtures, updateProviderFixtureAddress
} from "./fixture";
...
const PRIVATE_KEY_8_ANVIL_ACCOUNT = "0xdbda1821b80551c9d65939329250298aa3472ba22feea921c0cf5d620ea67b97"
...
describe.only("#getMatchedOffersByDealId", () => {
  ...
  let minDealDepositedEpoches: bigint;
  ...
  beforeAll(async () => {
    ...
    minDealDepositedEpoches = await coreContract.minDealDepositedEpoches()
  });

  test(
    `It checks that it matched successfully for 1:1 configuration where CC has status Active.`,
    async () => {
      ...
      let matchResult = await dealMatcherClient.getMatchedOffersByDealId(dealFixture.dealId)
      ...
      matchResult = await dealMatcherClient.getMatchedOffersByDealId(dealFixture.dealId)
  ...
  test(
    `It checks that it matched successfully with whitelisted Provider Offers even without CC.`,
    async () => {
      ...
      // Post-prepare fixtures.
      updateProviderFixtureAddress(signerAddress, [providerFixture])
      dealFixture.listAccess = [marketFixture.providerToBeMatched.providerAddress];
      ...
      const matchResult = await dealMatcherClient.getMatchedOffersByDealId(dealFixture.dealId)
      console.log('matchResult', matchResult)
      expect(matchResult.fulfilled).toEqual(true)
