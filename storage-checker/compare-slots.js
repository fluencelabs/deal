...
const compareSlots = (prev, curr) => {
  ...
  for (const slotAndOffset of currMap.keys()) {
    if (!prevMap.has(slotAndOffset)) {
      results.push(`New item at ${slotAndOffset}: ${currMap.get(slotAndOffset).type} ${currMap.get(slotAndOffset).label}`);
    }
  }
  // check deleted ones
  for (const slotAndOffset of prevMap.keys()) {
    if (!currMap.has(slotAndOffset)) {
      results.push(`Deleted item at ${slotAndOffset}: ${prevMap.get(slotAndOffset).type} ${prevMap.get(slotAndOffset).label}`);
    }
  }
  // compare others
  for (const slotAndOffset of prevMap.keys()) {
    if (currMap.has(slotAndOffset)) {
      ...
      if (prevLabel !== currLabel) {
        results.push(`Label changed for ${slotAndOffset}: ${prevLabel} -> ${currLabel}`);
      }
      const prevType = prevMap.get(slotAndOffset).type;
      const currType = currMap.get(slotAndOffset).type;
      if (prevType !== currType) {
        results.push(`Type changed for ${slotAndOffset}: ${prevType} ${prevLabel} -> ${currType} ${currLabel}`);
      }
    }
  }
  return results;
