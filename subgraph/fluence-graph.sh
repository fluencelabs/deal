...
while (($#)); do
  case "$1" in
    ...
    delete)
      action="$1"
      if [[ -z $3 ]]; then
          echo "For delete action subgraph name is required as 3d argument for the script."
          exit 1
      fi
      shift
      ;;
    ...
    -h|--help)
      ...
      ;;
    *)
      # If it is delete action we need parse subgraph name to delete.
      # Manually store subgraph name to delete (3d arg is only in this option).
      if [ ${action} == "delete" ] && [ -z ${subgraph_to_delete} ]; then
        subgraph_to_delete="$1"
        shift
      else
...
echo "subgraph_to_delete: ${subgraph_to_delete}"
