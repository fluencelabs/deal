...
import type { Core, Deal, IERC20 } from "../../typechain-types";
...
describe("ProviderManager", () => {
    ...
    it("removeProviderToken", async () => {
        ...
        const logRemoveTopic = (await removeTokenTx.wait()).logs.find(({ topics }: any) => topics[0] === removeEventTopic);

        expect(deal.interface.parseLog(logRemoveTopic!).args["id"]).to.be.equal(patId);
    });
