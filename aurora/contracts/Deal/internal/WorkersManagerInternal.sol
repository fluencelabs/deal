    ...
    IWorkersManagerInternal
{
    ...
    function _getPATOwner(IWorkersManager.PATId id) internal view override returns (address) {
        return _getPAT(id).owner;
    }

    function _createPAT(IWorkersManager.PATId id, address owner) internal override {
        ...
        if (status == IStatusControllerInternal.Status.WaitingForWorkers && currentWorkers >= _minWorkers()) {
    ...
    function _removePAT(IWorkersManager.PATId id) internal override {
        ...
        if (_status() == IStatusControllerInternal.Status.Working && currentWorkers < _minWorkers()) {
    ...
    function _getPAT(IWorkersManager.PATId id) private pure returns (PAT storage pat) {
