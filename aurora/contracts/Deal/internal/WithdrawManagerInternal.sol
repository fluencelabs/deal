...
abstract contract WithdrawManagerInternal is DealConfigInternal {
    ...
    function _getUnlockedAmountBy(IERC20 token, address owner, uint256 timestamp) internal view onlyFluenceToken(token) returns (uint256) {
        return _requests[owner].getAmountBy(timestamp - _core().withdrawTimeout());
    }

    function _createWithdrawRequest(IERC20 token, address owner, uint256 amount) internal onlyFluenceToken(token) {
        _requests[owner].push(amount);
    }

    function _withdraw(IERC20 token, address owner) internal onlyFluenceToken(token) {
